...
/// Knife-edge diffraction loss in dB (ITU-R P.526-15 §4.1 approximation).
///
/// For v > -0.78:
...
///
/// Logarithmic in v, so it stays physical in deep shadow (v = 20 -> ~39 dB),
/// unlike the Lee quadratic approximation which explodes past v ~ 2.4.
...
pub fn knife_edge_loss(v: f64) -> f64 {
    if v <= -0.78 {
        ...
        let t = v - 0.1;
        let loss = 6.9 + 20.0 * ((t * t + 1.0).sqrt() + t).log10();
        loss.max(0.0)
    }
}
...
mod tests {
    ...
    fn test_knife_edge_grazing() {
        ...
        // P.526: 6.9 + 20*log10(sqrt(1.01) - 0.1) = ~6.03 dB at grazing
        assert!((loss - 6.03).abs() < 0.05, "Grazing loss: {} dB", loss);
    }

    #[test]
    fn test_knife_edge_full_obstruction() {
        // P.526: v = 1 -> 6.9 + 20*log10(sqrt(0.81+1) + 0.9) = ~13.9 dB
        let loss = knife_edge_loss(1.0);
        assert!((loss - 13.9).abs() < 0.1, "v=1 loss: {} dB", loss);
    }

    #[test]
    fn test_knife_edge_deep_shadow_stays_physical() {
        // Deep shadow (v = 20) must be ~39 dB, not the ~700 dB the old
        // Lee quadratic produced.
        let loss = knife_edge_loss(20.0);
        assert!((loss - 39.0).abs() < 1.0, "v=20 loss: {} dB", loss);
        // Monotonic in v
        assert!(knife_edge_loss(30.0) > loss);
    }
