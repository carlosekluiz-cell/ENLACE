//! gRPC service handler implementations for the Pulso RF Engine.
//!
...
use pulso_propagation::models::itm::{itm_path_loss, ItmParams};
use pulso_propagation::models::p1812::P1812Model;
use pulso_propagation::models::p530::{compute_link_budget, LinkBudgetParams};
use pulso_propagation::models::tr38901::Tr38901RmaModel;
use pulso_propagation::models::{Environment, PathLossParams, PathLossResult, PropagationModel};
use pulso_propagation::vegetation::VegetationCorrector;
use pulso_propagation::AntennaPattern;

use pulso_optimizer::output::{optimize_tower_placement, OptimizationParams};

use pulso_terrain::cache::TileCache;
use pulso_terrain::profile::{extract_profile, GeoPoint, DEFAULT_K_FACTOR};

use crate::config::ServiceConfig;
use crate::proto;
use crate::proto::rf_engine_server::RfEngine;

/// The gRPC service implementation wrapping all Pulso RF engine functionality.
pub struct RfEngineService {
    #[allow(dead_code)]
    config: ServiceConfig,
    /// Shared bare-earth (DTM) tile cache protected by a mutex.
    tile_cache: Mutex<TileCache>,
    /// Surface-model (DSM) tile cache — buildings/vegetation included.
    dsm_tile_cache: Mutex<TileCache>,
    /// Bare-earth (ANADEM) tile cache — vegetation/building bias removed.
    ground_tile_cache: Mutex<TileCache>,
}
...
impl RfEngineService {
    ...
    pub fn new(config: ServiceConfig) -> Self {
        ...
        let dsm_tile_cache = TileCache::new(&config.dsm_tile_dir, config.tile_cache_size);
        let ground_tile_cache =
            TileCache::new(&config.ground_tile_dir, config.tile_cache_size);
        Self {
            config,
            tile_cache: Mutex::new(tile_cache),
            dsm_tile_cache: Mutex::new(dsm_tile_cache),
            ground_tile_cache: Mutex::new(ground_tile_cache),
        }
    }

    /// Sample the DTM tile cache into an in-memory raster covering the
    /// coverage area, for lock-free parallel ray sampling. Returns None if
    /// no tile provided any elevation (engine then runs terrain-free).
    fn build_terrain_grid(
        &self,
        center_lat: f64,
        center_lon: f64,
        radius_m: f64,
        ...
        // Cap raster size; coarsen the step on very large areas.
        const MAX_DIM: usize = 2048;
        let step_m = resolution_m.max(2.0 * radius_m / MAX_DIM as f64).max(30.0);

        let meters_per_deg_lat = 111_320.0;
        let meters_per_deg_lon = (111_320.0 * center_lat.to_radians().cos()).max(1.0);
        let lat_step = step_m / meters_per_deg_lat;
        let lon_step = step_m / meters_per_deg_lon;
        let lat_range = radius_m / meters_per_deg_lat;
        let lon_range = radius_m / meters_per_deg_lon;

        let lat_max = center_lat + lat_range;
        let lon_min = center_lon - lon_range;
        let height = ((2.0 * lat_range / lat_step).ceil() as usize + 1).min(MAX_DIM);
        let width = ((2.0 * lon_range / lon_step).ceil() as usize + 1).min(MAX_DIM);
        ...
        let mut any = false;
        {
            ...
            for row in 0..height {
                let lat = lat_max - row as f64 * lat_step;
                for col in 0..width {
                    let lon = lon_min + col as f64 * lon_step;
                    if let Some(elev) = cache.elevation(lat, lon) {
                        data[row * width + col] = elev as f32;
                        any = true;
                    }
                }
            }
...
/// Parse the wire environment string ("" defaults to rural).
fn parse_environment(s: &str) -> Result<Environment, Status> {
    match s.to_lowercase().as_str() {
        "urban" => Ok(Environment::Urban),
        "suburban" => Ok(Environment::Suburban),
        "rural" | "" => Ok(Environment::Rural),
        "open" | "open_rural" => Ok(Environment::OpenRural),
        other => Err(Status::invalid_argument(format!(
            "unknown environment '{}' (expected urban|suburban|rural|open)",
            other
        ))),
    }
}
...
impl RfEngine for RfEngineService {
    ...
    async fn compute_coverage(
        ...
        // Terrain shadowing: sample the DTM once into an in-memory raster.
        let terrain_grid =
            self.build_terrain_grid(req.tower_lat, req.tower_lon, req.radius_m, grid_res);
        if terrain_grid.is_none() {
            warn!("No terrain tiles for coverage area — computing without shadowing");
        }

        let cov_result =
            prop_compute_coverage(&tower, &area, grid_res, min_signal, terrain_grid.as_ref());
        ...
        let proto_stats = proto::CoverageStats {
            ...
            coverage_pct_p90: cov_result.stats.coverage_pct_p90,
            sigma_db: cov_result.stats.sigma_db,
        };
    ...
    async fn terrain_profile(
        ...
        // Pick the elevation surface: bare earth (default) or DSM.
        let cache_mutex = match req.surface.to_lowercase().as_str() {
            "dsm" => &self.dsm_tile_cache,
            "ground" => &self.ground_tile_cache,
            "dtm" | "" => &self.tile_cache,
            other => {
                return Err(Status::invalid_argument(format!(
                    "unknown surface '{}' (expected dtm|dsm|ground)",
                    other
                )))
            }
        };
