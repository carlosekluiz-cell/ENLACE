...
//   2. Infer splitter type: ≤32 ONTs → 1:32 (max 32), else 1:64 (max 64)
...
///
/// Groups ONT readings by port, counts unique serials, infers splitter type,
...
pub fn predict_splitter_capacity(readings: &[OntReading]) -> Vec<SplitterCapacity> {
    ...
    for (port, port_readings) in &by_port {
        ...
        // Infer splitter type
        let (splitter_type, max_ports) = if active_onts <= SPLITTER_32_MAX {
            ("1:32".to_string(), 32u32)
        } else {
            ("1:64".to_string(), 64u32)
        };
