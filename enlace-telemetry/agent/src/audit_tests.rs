...
mod audit {
  ...
  <data><xpon xmlns="urn:bbf:yang:bbf-xpon"><channel-terminations><channel-termination>
    ...
    /// 3A: 10 hard-offline ONTs on same port → FibreCut (no dying gasp)
    #[test]
    ...
    /// 3B: Only dying-gasp ONTs (power failure) → no event (excluded from hard-offline count)
    #[test]
    fn audit_3b_dying_gasp_only_no_event() {
        let cfg = make_fault_config();
        let mut detector = FaultDetector::new(&cfg);
        let mut onts = Vec::new();
        for i in 0..10 {
        ...
        let events = detector.check(&onts);
        assert!(events.is_empty(), "3B FAIL: dying-gasp-only should produce no event");

        println!("3B PASS: Dying-gasp-only ONTs correctly excluded (no fault event)");
    }
    ...
    fn audit_4a_linear_regression_downtrend() {
        ...
        // That's -0.1 dBm/day → Critical (threshold: -0.07)

        // Verify via classify_degradation
        let sev = crate::predictions::classify_degradation(-0.1, -23.0, None);
        assert_eq!(sev, Some(DegradationSeverity::Critical), "4A FAIL: -0.1 dBm/day should be Critical");

        println!("4A PASS: Linear regression downtrend");
        println!("  slope=-0.1 dBm/day → severity=Critical");
    }

    /// 4B: Classify degradation thresholds
    #[test]
    fn audit_4b_degradation_thresholds() {
        // No degradation (rate > -0.015)
        let none = crate::predictions::classify_degradation(-0.005, -22.0, None);
        assert_eq!(none, None, "4B: -0.005 should be None");

        // Watch: rate between -0.015 and -0.035
        let watch = crate::predictions::classify_degradation(-0.02, -22.0, None);
        assert_eq!(watch, Some(DegradationSeverity::Watch), "4B: -0.02 should be Watch");

        // Warning: rate between -0.035 and -0.07
        let warning = crate::predictions::classify_degradation(-0.04, -22.0, None);
        assert_eq!(warning, Some(DegradationSeverity::Warning), "4B: -0.04 should be Warning");

        // Critical rate: rate ≤ -0.07
        let critical = crate::predictions::classify_degradation(-0.08, -22.0, None);
        assert_eq!(critical, Some(DegradationSeverity::Critical), "4B: -0.08 should be Critical");

        println!("4B PASS: Degradation thresholds");
        println!("  -0.005 → None");
        println!("  -0.020 → Watch");
        println!("  -0.040 → Warning");
        println!("  -0.080 → Critical");
    }
    ...
    fn audit_4e_custom_thresholds() {
        let custom = DegradationConfig {
            ...
            watch_threshold_db: -0.01,     // tighter than default
            warning_threshold_db: -0.025,
            critical_threshold_db: -0.05,
            min_critical_rx_dbm: -26.0,     // tighter than default -27.0
        };

        // -0.015 would be Watch with defaults, but with custom -0.01 threshold it's also Watch
        let sev = crate::predictions::classify_degradation(-0.015, -22.0, Some(&custom));
        assert_eq!(sev, Some(DegradationSeverity::Watch), "4E FAIL: -0.015 with custom threshold");

        // -0.03 would be Watch with defaults, but Warning with custom -0.025
        let sev2 = crate::predictions::classify_degradation(-0.03, -22.0, Some(&custom));
        assert_eq!(sev2, Some(DegradationSeverity::Warning), "4E FAIL: -0.03 with custom");
        ...
        println!("  custom watch=-0.01, warning=-0.025, critical=-0.05, min_rx=-26.0");
        println!("  -0.015 → Watch");
        println!("  -0.030 → Warning");
...
"#;
        ...
        let sev = crate::predictions::classify_degradation(-0.02, -22.0, None);
        assert_eq!(sev, Some(DegradationSeverity::Watch));
        let sev2 = crate::predictions::classify_degradation(-0.04, -22.0, None);
        assert_eq!(sev2, Some(DegradationSeverity::Warning));
        let sev3 = crate::predictions::classify_degradation(-0.08, -22.0, None);
