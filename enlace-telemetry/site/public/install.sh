...
AGENT_VERSION="0.1.0"
BASE_URL="https://enlace.network/releases"
...
ARCH=$(uname -m)
case "$ARCH" in
    x86_64)  BINARY="pulso-agent-x86_64-linux" ;;
    aarch64|armv7l)
        echo "No prebuilt binary is published for $ARCH yet — only x86_64."
        echo "Build from source (cargo build --release) or contact enlace.network."
        exit 1 ;;
    *)       echo "Unsupported architecture: $ARCH"; exit 1 ;;
esac

echo "Detected: $ARCH"
echo "Downloading Pulso Agent v${AGENT_VERSION}..."

# Download binary + checksums
curl -fSL "${BASE_URL}/${BINARY}" -o /tmp/pulso-agent
curl -fSL "${BASE_URL}/SHA256SUMS" -o /tmp/pulso-agent.SHA256SUMS

# Verify checksum before anything is installed
EXPECTED=$(awk -v bin="$BINARY" '$2 == bin { print $1 }' /tmp/pulso-agent.SHA256SUMS)
ACTUAL=$(sha256sum /tmp/pulso-agent | awk '{ print $1 }')
if [ -z "$EXPECTED" ] || [ "$EXPECTED" != "$ACTUAL" ]; then
    echo "✗ Checksum verification FAILED for ${BINARY}"
    echo "  expected: ${EXPECTED:-<missing from SHA256SUMS>}"
    echo "  actual:   ${ACTUAL}"
    rm -f /tmp/pulso-agent /tmp/pulso-agent.SHA256SUMS
    exit 1
fi
rm -f /tmp/pulso-agent.SHA256SUMS
