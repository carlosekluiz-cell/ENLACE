...
**Open-source, vendor-agnostic ISP network intelligence agent built in Rust.**

Monitors OLTs (Huawei, ZTE, FiberHome, Datacom, Intelbras, Parks, BDCOM, VSOL, CDATA, Ubiquiti), MikroTik routers, RADIUS sessions, and TR-069 CPE — feeding real-time telemetry to the [Enlace Network](https://enlace.network) intelligence platform.
...
128,000+ ISPs in Brazil (and 100,000+ worldwide) manage their networks blind. The data exists — in OLTs, routers, RADIUS servers — but it's locked in vendor silos. No platform combines operational telemetry with market intelligence.

**Pulso Agent bridges that gap.** It reads from your existing equipment using standard protocols (SNMP, SSH, RouterOS API, RADIUS, TR-069) and sends normalized telemetry to Pulso Network, where it's fused with 37+ public data sources for actionable intelligence.

## Quick Start (60 seconds)

```bash
# Download
curl -sL https://enlace.network/install.sh | bash
