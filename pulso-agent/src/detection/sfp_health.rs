...
// Sensor-resolution math (why the thresholds sit where they do):
// SFP DDM rx power quantizes at ~0.1 dB (SFF-8472 LSB) with a ±0.5-1 dB
// diurnal thermal swing. Averaging all ONTs on a port into daily means
// reduces quantization noise, but the smallest TOTAL drop distinguishable
// from noise remains ~3x the quantization step = 0.3 dB. Over the minimum
// 7-day window that is ~0.3 dB / week; any weekly-slope threshold below
// that (the old Watch at -0.01 dBm/week was 10x below the sensor LSB)
// classifies pure noise. Thresholds are therefore >= 0.1 dBm/week AND a
// trend is only quoted when the fitted total drop across the observed
// window exceeds 0.3 dB.

/// Minimum fitted total drop (dB) across the window for a trend to be real.
/// 3x the ~0.1 dB DDM quantization step.
const MIN_TOTAL_DROP_DB: f64 = 0.3;

/// Slope threshold (dBm/week): slopes flatter than this are Healthy.
const SLOPE_HEALTHY_LIMIT: f64 = -0.1;

/// Slope threshold (dBm/week): slopes between this and HEALTHY are Watch.
const SLOPE_WATCH_LIMIT: f64 = -0.2;

/// Slope threshold (dBm/week): slopes between this and WATCH are Warning.
const SLOPE_WARNING_LIMIT: f64 = -0.4;
...
const MIN_DAILY_SAMPLES: usize = 7;

/// Minimum elapsed window (days) before any trend is quoted.
...
pub fn analyze_sfp_health(readings: &[OntReading]) -> Vec<SfpHealth> {
    ...
    for (port, port_readings) in &by_port {
        let online_readings: Vec<&&OntReading> = port_readings
            .iter()
            .filter(|r| r.status == OntReadingStatus::Online && super::sane_rx(r).is_some())
            .collect();
        ...
        let window_days = daily_avgs
            .last()
            .map(|(d, _)| d - daily_avgs[0].0)
            .unwrap_or(0.0);

        if daily_avgs.len() < MIN_DAILY_SAMPLES || window_days < MIN_WINDOW_DAYS {
            // Not enough data to quote a trend; report the measured level only
            // (a genuinely low absolute Rx is a measurement, not a forecast).
        ...
        // Resolution gate: the fitted total drop over the observed window
        // must exceed 3x the DDM quantization step, otherwise the "trend"
        // is indistinguishable from sensor noise and is treated as flat.
        let total_fitted_drop = -slope_per_day * window_days;
        let slope_per_week = if total_fitted_drop > MIN_TOTAL_DROP_DB {
            slope_per_day * 7.0
        } else {
            0.0
        };
...
mod tests {
    ...
    #[test]
    fn test_sfp_quantization_noise_stays_healthy() {
        // ±0.1 dB quantization steps + a ±0.5 dB diurnal sine: no real trend.
        // Must classify Healthy with no ETA — this exact noise floor used to
        // trip the old -0.01 dBm/week Watch threshold.
        let mut readings = Vec::new();
        let serials = onts("Q", 10);
        for day in 0..14u32 {
            for hour in (0..24).step_by(6) {
                let ts = Utc.with_ymd_and_hms(2026, 1, 1 + day, hour, 0, 0).unwrap();
                // Diurnal thermal swing
                let diurnal = 0.5 * (2.0 * std::f64::consts::PI * hour as f64 / 24.0).sin();
                for (i, serial) in serials.iter().enumerate() {
                    // Quantize to the 0.1 dB DDM step, with per-ONT phase noise
                    let raw = -20.0 + diurnal + ((day + i as u32) % 2) as f64 * 0.1;
                    let quantized = (raw * 10.0).round() / 10.0;
                    readings.push(make_reading(serial, "OLT09/0/0/0", ts, Some(quantized), Some(500)));
                }
            }
        }
        // Sibling port for outlier comparison
        readings.extend(generate_stable_readings(&serials, "OLT09/0/1/0", -20.0, 14));

        let results = analyze_sfp_health(&readings);
        let noisy = results.iter().find(|r| r.port == "OLT09/0/0/0").unwrap();
        assert_eq!(
            noisy.severity,
            SfpSeverity::Healthy,
            "quantization + diurnal noise must not be classified as degradation (trend {})",
            noisy.rx_trend_per_week
        );
        assert!(
            noisy.estimated_weeks_to_failure.is_none(),
            "noise must not produce a failure ETA"
        );
    }
    ...
    fn test_sfp_single_port_degrading() {
        ...
        // Port 2 degrading at -0.25 dBm/week = -0.25/7 dBm/day
        // (0.71 dB total over 21 days — well above the 0.3 dB resolution gate)
        let slope_per_day = -0.25 / 7.0;
    ...
    fn test_sfp_weeks_to_failure() {
        ...
        // Port degrading at -0.3 dBm/week from -22 dBm (0.9 dB over 21 days,
        // above the resolution gate). Failure at -28 dBm -> ~19 weeks left.
        let slope_per_day = -0.3 / 7.0;
        readings.extend(generate_degrading_readings(
        ...
        // avg_rx is around -22.4 dBm (midpoint of 21-day degradation from -22),
        // failure at -28 -> ~5.6 dBm remaining at ~0.3 dBm/week -> ~19 weeks
        assert!(
