...
// Diurnal-Pattern Correlation Detector ("weather" module)
//
// HONESTY NOTE: this module consumes NO weather data. What it detects are
// DIURNAL-PATTERN CORRELATIONS in ONT signal readings — time-of-day-shaped
// group behaviour that is *consistent with* weather/environmental causes
// but is not confirmed by any rainfall/temperature feed. Every output is
// therefore described as a diurnal-pattern correlation with a suspected
// cause, never as an observed weather event. Patterns:
//
//   1. NIGHTTIME CONDENSATION (suspected) — Rx power worse at night than
//      day, consistent with moisture ingress into a splice enclosure.
//
//   2. PERIODIC RAIN INGRESS (suspected) — Coordinated degradation episodes
//      lasting 2-8 hours then full recovery, consistent with rainfall.
//
//   3. THERMAL EXPANSION (suspected) — Higher Rx variance during afternoon
//      vs stable nighttime readings, consistent with aerial thermal stress.
//
...
//
// Day/night boundaries are computed in the operator's local time via a
// configurable UTC offset (default 0 = UTC). Hardcoded UTC would put the
// "night" window in the wrong place in BST (UTC+1) or Brazil (UTC-3).
...
/// A detected diurnal-pattern correlation with a suspected environmental cause.
///
/// NOTE: `pattern` names the SUSPECTED cause; no weather data is consumed.
...
/// Suspected cause of a diurnal-pattern correlation (not confirmed by any
/// weather feed).
...
/// Minimum number of ONTs in a distance group to flag a pattern.
...
/// SFP DDM readings quantize at ~0.1 dB (SFF-8472 LSB) with ±0.5-1 dB
/// diurnal thermal swing. No group delta below 3x the quantization step
/// (0.3 dB) is distinguishable from sensor noise, so every dBm threshold
/// in this module sits at or above 0.3 dB.
const SENSOR_RESOLUTION_DB: f64 = 0.1;

/// Minimum day/night Rx power difference (dBm) to flag condensation.
/// 3x sensor resolution — the old 0.15 dBm threshold was below what the
/// DDM hardware can resolve.
const MIN_CONDENSATION_DELTA_DBM: f64 = 3.0 * SENSOR_RESOLUTION_DB;

/// Minimum degradation (dBm) from baseline to flag a rain episode
/// (3x sensor resolution).
const MIN_RAIN_DEGRADATION_DBM: f64 = 3.0 * SENSOR_RESOLUTION_DB;
...
/// Minimum afternoon stdev (dBm) for a thermal flag: variance below the
/// DDM quantization step is indistinguishable from noise regardless of
/// the afternoon/night ratio.
const MIN_THERMAL_STDEV_DBM: f64 = SENSOR_RESOLUTION_DB;
...
/// Detect diurnal-pattern correlations using UTC day/night boundaries.
/// For plants outside UTC use [`detect_weather_correlation_with_offset`].
pub fn detect_weather_correlation(readings: &[OntReading]) -> Vec<WeatherCorrelation> {
    detect_weather_correlation_with_offset(readings, 0)
}

/// Detect diurnal-pattern correlations from ONT readings, with day/night
/// boundaries computed at UTC + `utc_offset_hours` (operator local time).
///
...
/// shaped patterns. Only flags patterns affecting 3+ ONTs to confirm
/// infrastructure issues rather than single-ONT faults. No weather data is
/// consumed — see the module header.
pub fn detect_weather_correlation_with_offset(
    readings: &[OntReading],
    utc_offset_hours: i32,
) -> Vec<WeatherCorrelation> {
    ...
    for group in &groups {
        ...
        if let Some(correlation) = check_condensation(group, &by_serial, utc_offset_hours) {
        ...
        if let Some(correlation) = check_thermal_expansion(group, &by_serial, utc_offset_hours) {
            results.push(correlation);
        }
    }
...
/// Hour of day at the operator's local time (UTC + offset).
fn local_hour(ts: DateTime<Utc>, utc_offset_hours: i32) -> u32 {
    (ts + chrono::Duration::hours(utc_offset_hours as i64)).hour()
}

/// Check for a nighttime-condensation-shaped diurnal pattern.
/// Night (22:00-06:00 local) Rx consistently worse than day (10:00-18:00 local).
fn check_condensation(
    ...
    for serial in &group.serials {
        ...
        let night_rx: Vec<f64> = readings
            .iter()
            .filter(|r| {
                let h = local_hour(r.timestamp, utc_offset_hours);
                r.status == OntReadingStatus::Online && (h >= 22 || h < 6)
            })
            .filter_map(|r| sane_rx(r))
            .collect();
    ...
    Some(WeatherCorrelation {
        ...
        description: format!(
            "Diurnal-pattern correlation (suspected nighttime condensation): {} ONTs at {}–{}m \
             show avg {:.2} dBm worse Rx at night (22:00–06:00 local) vs day (10:00–18:00 local), \
             consistent with moisture ingress in a splice enclosure or cable joint. \
             No weather data consumed — time-of-day inference only.",
            flagged_count, group.min_distance, group.max_distance, avg_delta
        ),
    })
}

/// Check for a periodic-rain-ingress-shaped pattern.
...
fn check_rain_ingress(
    ...
    // Filter episodes: must last 2-8 hours. Shorter clusters (including
    // single-sample blips) are quantization/transient noise; longer ones are
    // persistent faults, not rain-shaped episodes. Only episodes passing
    // this filter count — the old code took max(filtered, unfiltered),
    // which made the filter a no-op.
    ...
    let episode_count = valid_episodes.len();
    ...
    Some(WeatherCorrelation {
        ...
        description: format!(
            "Diurnal-pattern correlation (suspected rain ingress): {} coordinated 2–8h \
             degradation episodes across {} ONTs at {}–{}m, rate {:.1}/week — check cable \
             joints and splice enclosures for water seals. No weather data consumed — \
             episodic pattern inference only.",
            episode_count, group.serials.len(), group.min_distance, group.max_distance, weekly_rate
        ),
    })
}

/// Check for a thermal-expansion-shaped diurnal pattern.
/// Afternoon (11:00-15:00 local) Rx stdev > 2x night (00:00-06:00 local) stdev,
/// with the afternoon stdev itself above the DDM quantization step.
fn check_thermal_expansion(
    ...
    for serial in &group.serials {
        ...
        // Afternoon readings (11:00-15:00 local)
        ...
        // Night readings (00:00-06:00 local)
        let night_rx: Vec<f64> = readings
            .iter()
            .filter(|r| {
                let h = local_hour(r.timestamp, utc_offset_hours);
        ...
        // Variance below the sensor quantization step (~0.1 dB) is noise,
        // whatever the afternoon/night ratio says.
        if afternoon_stdev < MIN_THERMAL_STDEV_DBM {
            continue;
        }

        if night_stdev < 1e-9 {
            // Night is perfectly stable; measurable afternoon variation counts
            flagged_count += 1;
    ...
    Some(WeatherCorrelation {
        ...
        description: format!(
            "Diurnal-pattern correlation (suspected thermal expansion): {} ONTs at {}–{}m \
             show {:.1}x higher Rx variance during afternoon (11:00–15:00 local) vs night \
             (00:00–06:00 local), consistent with aerial cable thermal stress. \
             No weather data consumed — time-of-day inference only.",
...
mod tests {
        ...
        for day in 0..7 {
            for hour_idx in 0..6 {
                ...
                for ont_idx in 0..4 {
                    ...
                    // Night readings are 0.5 dBm worse (above the 0.3 dB
                    // sensor-resolution threshold)
                    let rx = if is_night { -20.5 } else { -20.0 };
    ...
    fn test_weather_periodic_rain() {
        ...
        // Two genuine rain-shaped episodes: hourly coordinated 0.6 dBm dips
        // lasting 4 hours, then full recovery.
        let episode1 = (4u32, 14u32..=18); // March 5, 14:00-18:00
        let episode2 = (9u32, 10u32..=14); // March 10, 10:00-14:00
        let in_episode = |day: u32, hour: u32| {
            (day == episode1.0 && episode1.1.contains(&hour))
                || (day == episode2.0 && episode2.1.contains(&hour))
        };

        for day in 0..14u32 {
            for hour in 0..24u32 {
                // Baseline every 2 hours; hourly resolution during episodes
                // so the episode forms one contiguous 4h cluster.
                let episodic = in_episode(day, hour);
                if !episodic && hour % 2 != 0 {
                    continue;
                }
                let ts = Utc.with_ymd_and_hms(2026, 3, 1 + day, hour, 0, 0).unwrap();
                let rx = if episodic { -20.6 } else { -20.0 };
    ...
    #[test]
    fn test_weather_rain_filter_rejects_wrong_durations() {
        // Episodes OUTSIDE the 2-8h window must not count: two single-sample
        // blips and one 12-hour sustained dip. The old code took
        // max(filtered, unfiltered) which let these through.
        let port = "0/6/0";
        let mut readings = Vec::new();
        let serials: Vec<String> = (0..4).map(|i| format!("BLIP{:03}", i)).collect();

        for day in 0..14u32 {
            for hour in (0..24u32).step_by(2) {
                let ts = Utc.with_ymd_and_hms(2026, 3, 1 + day, hour, 0, 0).unwrap();
                // Two isolated single-sample blips (0h clusters)
                let blip = (day == 3 && hour == 12) || (day == 7 && hour == 8);
                // One 12-hour sustained dip on day 10 (persistent fault shape)
                let sustained = day == 10 && (6..=18).contains(&hour);
                let rx = if blip || sustained { -20.6 } else { -20.0 };
                for (i, serial) in serials.iter().enumerate() {
                    let distance = 900 + i as u32 * 10;
                    readings.push(make_reading(serial, port, ts, Some(rx), Some(distance)));
                }
            }
        }
        ...
        assert!(
            !correlations
                .iter()
                .any(|c| c.pattern == WeatherPattern::PeriodicRainIngress),
            "blips and 12h dips are not 2-8h rain episodes: {:?}",
            correlations
        );
    }

    #[test]
    fn test_weather_condensation_below_resolution_suppressed() {
        // A 0.15 dBm day/night delta is below the 0.3 dB (3x DDM resolution)
        // floor and must not be flagged, however consistent it looks.
        let port = "0/7/0";
        let mut readings = Vec::new();
        for day in 0..7 {
            for hour_idx in 0..6 {
                ...
                let is_night = hour >= 22 || hour < 6;
                for ont_idx in 0..4 {
                    let serial = format!("SUB{:03}", ont_idx);
                    let distance = 500 + ont_idx * 20;
                    let rx = if is_night { -20.15 } else { -20.0 };
                    readings.push(make_reading(&serial, port, ts, Some(rx), Some(distance)));
                }
            }
        }
        ...
        assert!(
            !correlations
                .iter()
                .any(|c| c.pattern == WeatherPattern::NighttimeCondensation),
            "0.15 dBm delta is below sensor resolution: {:?}",
            correlations
        );
    }

    #[test]
    fn test_weather_utc_offset_moves_night_window() {
        // Same physical pattern as the condensation test but timestamps are
        // stored in UTC for a UTC-3 plant: local night 22:00-06:00 is UTC
        // 01:00-09:00. With offset -3 the pattern is found; without it the
        // day/night split lands in the wrong place and the delta shrinks.
        let port = "0/8/0";
        let mut readings = Vec::new();
        for day in 0..7 {
            for hour_idx in 0..12 {
                let utc_hour = hour_idx * 2;
                let ts = Utc.with_ymd_and_hms(2026, 3, 10 + day, utc_hour, 0, 0).unwrap();
                let local_hour = (utc_hour + 24 - 3) % 24;
                let is_local_night = local_hour >= 22 || local_hour < 6;
                for ont_idx in 0..4 {
                    let serial = format!("TZ{:03}", ont_idx);
                    let distance = 700 + ont_idx * 20;
                    let rx = if is_local_night { -20.5 } else { -20.0 };
                    readings.push(make_reading(&serial, port, ts, Some(rx), Some(distance)));
                }
            }
        }

        let with_offset = detect_weather_correlation_with_offset(&readings, -3);
        assert!(
            with_offset
                .iter()
                .any(|c| c.pattern == WeatherPattern::NighttimeCondensation),
            "UTC-3 offset should recover the local-night pattern: {:?}",
            with_offset
        );
    }
        ...
        for day in 0..7 {
            for hour in (0..24).step_by(2) {
                ...
                for ont_idx in 0..4 {
                    ...
                    // Afternoon: high variance (±0.4 dB, well above the
                    // 0.1 dB quantization floor). Night: stable (-20.0).
                    let rx = if is_afternoon {
                        if (day + hour + ont_idx) % 2 == 0 { -19.6 } else { -20.4 }
                    } else {
                        -20.0
                    };
                    readings.push(make_reading(&serial, port, ts, Some(rx), Some(distance)));
