...
// Datacom OLT Collector (DmOS — DM4610, DM4615)
//
// Enterprise OID: 1.3.6.1.4.1.3709
// MIB: GPON-ONU-IF-MIB (.3709.3.6.2.1.1)
//
// Collects via SNMP v2c:
//   - ONT description, oper status, RX/TX power, traffic (GPON-ONU-IF-MIB)
//   - Interface data (standard IF-MIB)
//   - OLT system info via SNMPv2-MIB
...
impl DatacomCollector {
    ...
    async fn collect_onts_snmp(&self) -> anyhow::Result<Vec<OntData>> {
        ...
        // GPON-ONU-IF-MIB tables
        let descrs = snmp.walk_table(oids::datacom::ONT_DESCR).await.unwrap_or_default();
        let oper_statuses = snmp.walk_table(oids::datacom::ONT_OPER_STATUS).await.unwrap_or_default();
        let rx_powers = snmp.walk_table(oids::datacom::ONT_RX_POWER).await.unwrap_or_default();
        let tx_powers = snmp.walk_table(oids::datacom::ONT_TX_POWER).await.unwrap_or_default();
        let aliases = snmp.walk_table(oids::datacom::ONT_ALIAS).await.unwrap_or_default();
        let in_octets = snmp.walk_table(oids::datacom::ONT_IN_OCTETS).await.unwrap_or_default();
        let out_octets = snmp.walk_table(oids::datacom::ONT_OUT_OCTETS).await.unwrap_or_default();

        // Use descr table for primary enumeration; fall back to oper_status
        let primary = if !descrs.is_empty() { &descrs } else { &oper_statuses };
        if primary.is_empty() {
            return Ok(Vec::new());
        }
        ...
        for (idx, entry) in primary.iter().enumerate() {
            ...
            // Use alias (customer name) or description as serial
            let serial = super::snmp_helper::find_by_suffix(&aliases, &index)
                ...
                .unwrap_or_else(|| format!("datacom-{}", index));
            ...
            onts.push(OntData {
                ...
                distance_meters: None, // Datacom GPON-ONU-IF-MIB doesn't expose distance
                vendor_id: None, equipment_id: None, firmware_version: None,
                in_octets: in_oct, out_octets: out_oct, eth_speed_mbps: None,
            });
        }
        Ok(onts)
    }
...
impl OltCollector for DatacomCollector {
    ...
    async fn collect(&self) -> anyhow::Result<OltData> {
        ...
        let uplinks = self.collect_interfaces_ifmib().await.unwrap_or_default();
