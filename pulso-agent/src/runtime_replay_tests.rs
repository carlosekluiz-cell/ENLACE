...
mod runtime_replay {
    ...
    fn make_replay_olt(nodes: &[RealSeedNode], faulted: bool) -> OltData {
        let port_id = "replay/pon0";
        let mut onts = Vec::new();
        for (idx, node) in nodes.iter().enumerate() {
            let serial = make_serial("REPLAY", node, idx + 1);
            let is_faulted = faulted && idx >= 3 && idx < 9;
    ...
    async fn runtime_replay_exercises_live_cycle_path() {
        ...
        let mut diag_state = crate::diagnostics::OfflineAlertState::default();
        let topologies = crate::TopologyRegistry::from_config(None);
        let agent_metrics = crate::metrics::Metrics::new();
        ...
        let baseline_olt = make_replay_olt(&replay_nodes, false);
        let olt = make_replay_olt(&replay_nodes, true);
        ...
        let webhooks = std::sync::Arc::new(WebhookDispatcher::new(&[WebhookConfig {
            url: server_url.clone(),
        ...
        // (transition detection means mass-offline only alarms after this)
        let mut baseline_telemetry = TelemetryPayload::new(&cfg.agent_id);
        let baseline_artifacts = process_olt_cycle(
            &cfg,
            &db,
            &mut baseline_telemetry,
            Some(&mut detector),
            &mut diag_state,
            &topologies,
            Some(&elastic),
        ...
        assert!(
            baseline_artifacts.fault_events.is_empty(),
            "baseline cycle must not produce fault events"
        );

        // Cycle 2: 6 ONTs transition to offline — one incident opens
        ...
        // Webhook dispatch is detached (fire-and-forget) — give it time to land
