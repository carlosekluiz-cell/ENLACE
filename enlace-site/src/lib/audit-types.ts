...
export interface AuditResult {
  ...
  impact: Impact;
  onts: OntData[];
  /** CSV import honesty counters — present on newer audit server responses. */
  import_report?: ImportReport;
}

/** Client-safe summary of how the uploaded CSV was parsed. */
export interface ImportReport {
  /** Rows successfully converted into readings. */
  rows_ok: number;
  /** Rows dropped (structural errors, bad timestamps, missing serials). */
  rows_skipped: number;
  /** Sanitized samples of skip reasons (capped server-side). */
  skip_samples: string[];
  ...
  /** Distinct unrecognized status values seen (up to 10). */
  unknown_status_values: string[];
  /** True when the file had no timestamp column (snapshot export). */
